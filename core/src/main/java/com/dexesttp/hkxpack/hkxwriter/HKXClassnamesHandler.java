...
class HKXClassnamesHandler {
	...
	ClassnamesData getClassnames(HKXFile file) {
		...
		data.put(0, "hkClass", ByteUtils.fromLong(hkClassID, 4));
		data.put(1, "hkClassMember", ByteUtils.fromLong(hkClassMemberID, 4));
		data.put(2, "hkClassEnum", ByteUtils.fromLong(hkClassEnumID, 4));
		data.put(3, "hkClassEnumItem", ByteUtils.fromLong(hkClassEnumItemID, 4));
		int i = 4;
